...
describe('formatTree', () => {
  ...
  it('formats a single file at the root with the └── connector', () => {
    ...
    expect(lines).toEqual([
      'aaa',
      '└── index.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
    ]);
  });
  ...
  it('uses ├── for non-last siblings and └── for the last one', () => {
    ...
    expect(lines).toEqual([
      'aaa',
      '├── a.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
      '├── b.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
      '└── c.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
    ]);
  });
  ...
  it('sorts folders before files at the same level', () => {
    ...
    expect(lines).toEqual([
      ...
      '├── bbb',
      '│   └── index.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
      '└── z.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
    ]);
  });
  ...
  it('propagates │ prefix through deeply nested folders', () => {
    ...
    expect(lines).toEqual([
      ...
      '├── bbb',
      '│   └── ccc',
      '│       └── deep.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
      '└── other.ts — 10 lines, 1 function, 0 classes, 0 methods, 0 interfaces',
    ]);
  });
  ...
  it('injects color codes from config around folder and metric segments', () => {
    ...
    expect(lines).toEqual([
      '<F>aaa<R>',
      '└── index.ts — <M>10 lines, 1 function, 0 classes, 0 methods, 0 interfaces<R>',
    ]);
  });
});
